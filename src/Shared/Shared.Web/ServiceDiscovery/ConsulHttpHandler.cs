...
internal sealed class ConsulHttpHandler : DelegatingHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.RequestUri is null) return await base.SendAsync(request, cancellationToken);

        var serviceName = request.RequestUri.Host;
        if (string.IsNullOrWhiteSpace(serviceName)) return await base.SendAsync(request, cancellationToken);

        var filter = _selector == serviceName;
        var services = await _client.Agent.Services(filter, cancellationToken);
        if (!services.Response.Any()) throw new ServiceNotFoundException(serviceName);
