...
public class UserRequestedBetPlacementHandler : IEventHandler<UserRequestedBetPlacement>
{
    ...
    public UserRequestedBetPlacementHandler(ILogger<UserRequestedBetPlacementHandler> logger, SlipContext slipContext,
        Event.EventClient eventGrpcClient)
    {
        _logger = logger;
    ...
    {
        _logger.LogInformation("User {UserId} requested bet placement for Slip {SlipId}", @event.UserId,
            @event.Slip.Id);
