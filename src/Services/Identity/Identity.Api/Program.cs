...
app.MapPost("api/v1/identity/sign-in", async (SignIn command, IDispatcher dispatcher, ITokenStorage storage) =>
    {
...
app.MapGet("api/v1/identity/me", async (IDispatcher dispatcher, HttpContext context) =>
{
...
app.MapGet("api/v1/identity/users/{id:guid}", async (Guid id, IDispatcher dispatcher) =>
{
...
{
    return string.IsNullOrWhiteSpace(context.User.Identity?.Name) ? Guid.Empty : Guid.Parse(context.User.Identity.Name);
}
